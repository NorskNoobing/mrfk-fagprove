...
param (
    ...
    #Set VmName
    [string]$VmName = "$($env:USERNAME)_PengebingenAs-file01",
    #Set path for the VMs disk
    [string]$VhdPath = "$VmPath\disk\$VmName.vhdx",
    #Set path for the extra disk for the fileserver
    [string]$FileshareVhdPath = "$VmPath\disk\$($VmName)_fileshare.vhdx"
)
...
#Set all New-VHD function params
$splat = @{
    #Hostname of the computer running the Hyper-V service
    "ComputerName" = $ComputerName
    #Path where you'd like to store the virtual disk (vhdx file)
    "Path" = $FileshareVhdPath
    #Size of the new virtual disk
    "SizeBytes" = "10GB"
}
#Run function that creates a new virtual disk (vhdx file)
$null = New-VHD @splat

#Set all Add-VMHardDiskDrive function params
$splat = @{
    #Hostname of the computer running the Hyper-V service
    "ComputerName" = $ComputerName
    #Name of the VM
...
#Run function that adds an extra drive to the VM
Add-VMHardDiskDrive @splat

#Set all Start-VM function params
$splat = @{
    #Hostname of the computer running the Hyper-V service
    "ComputerName" = $ComputerName
    #Name of the VM
...
#Run function that starts the VM
Start-VM @splat